<?php
 $aux = 0;
?>
<p class="tituloPrincipalPag tituloGrande">Sugerencias</p>
                ...
                @if($proyecto->pro_estado == $sugerencia->proyecto_estado)
                    ...
                    @if($continuar)
                        ...
                        <li class="collection-item avatar">
                            <a href="/inv/sugerencia/{{\Illuminate\Support\Facades\Crypt::encrypt($sugerencia->id)}}">
                                  <p class="title teal-text"><strong>Proyecto: </strong>{{$sugerencia->proyecto->pro_titulo}}</p>
                                  <p class="texto-informacion-medium black-text"><strong>Tipo Sugerencia: </strong>{{$sugerencia->sug_elemento_nombre}}</p>
                                  <p class="texto-informacion-medium black-text"><strong>Fecha: </strong>{{date('Y-m-d',strtotime($sugerencia->created_at))}}</p>
